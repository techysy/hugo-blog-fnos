...
- 📝 **写文章自动渲染** — 在数据目录的 `content/post/` 放 markdown，Hugo 自动生成站点
...
在数据目录的 `content/post/` 下新建 markdown 文件：
...
把主题放到 `/vol4/@appdata/hugo-blog/blog/themes/<name>/`，并在 `config/_default/config.toml` 改 `theme = "<name>"`。
